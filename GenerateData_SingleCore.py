...
def load_data():
    ...
    for i in range(0, len(os.listdir(path + folders[args.num_of_dogs - 1]))):
        ...
        if 'interictal' in segment:
            generate_spectrograms(data, folders[args.num_of_dogs - 1], 'interictal_', k)
            k += 1
        elif 'preictal' in segment:
            generate_spectrograms(data, folders[args.num_of_dogs - 1], 'preictal_', l)
            l += 1
        print ('File %s completed.' %(k+l))
